...
try { if (!fetch) fetch = require('node-fetch'); } catch (e) { /* node-fetch not installed, rely on global fetch */ }
...
if (samples.length === 0) {
  ...
  console.log('  node scripts/e2e_tts_stt.cjs [--transcribe-url URL] [--tts-url URL] sample1.wav[:lang] sample2.webm[:lang]');
  console.log('\nDefaults:');
