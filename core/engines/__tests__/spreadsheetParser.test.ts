...
describe('spreadsheet parser', () => {
  it('parses a simple workbook buffer and infers types', () => {
    ...
    const sheet = info.sheets.find(s => s.name === 'Sheet1');
    expect(sheet).toBeTruthy();
    expect(sheet!.headers).toEqual(['Product', 'Price', 'Date']);
    const priceCol = sheet!.columns.find(c => c.header === 'Price');
    expect(priceCol).toBeTruthy();
    expect(priceCol!.inferredType).toBe('number');
    const dateCol = sheet!.columns.find(c => c.header === 'Date');
    expect(['date','string','mixed']).toContain(dateCol!.inferredType);
  });
});
